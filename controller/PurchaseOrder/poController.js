...
'Error while Inserting po record!',
500
);
return next(error);
}
...
await pool.query(
`DELETE FROM po  WHERE sPOGUID = ? `, 
[sPOGUID],
);
} catch (err) {
const error = new ErrorHandler(
'Error while Deleting po record!',
200
);
return next(error);
}
...
let query = 'SELECT po.dDate, po.dtCreatedOn, po.nPrice, po.nQty, po.sCreatedBy, po.sCustGUID, po.sItemName, po.sPOGUID, po.sRef From po '
...
const [rows] = await pool.query(query, values);
        ...
        const error = new ErrorHandler(
            'Error while Selecting po record!',
            500
        );
        return next(error);
    }
...
const error = new ErrorHandler(
'Error while Updating po record!',
500
);
return next(error);
}
